...
- The output template must require an "Awareness Stage & Messaging Map" section inside the ICP itself (per-sub-avatar awareness level, what you can say, what you must not say, angle direction). The schwartz-awareness-mapper output has no named home in the 11-section list, so it gets improvised each run even though it is a required carried-forward artifact every downstream skill keys off.
- The sub-avatar heading dash (`### Name — descriptor`) is a load-bearing em dash the app parser depends on and is the single exception to the global no-em-dash rule. Encode the exception explicitly so runs neither strip the required dash nor leak stray em dashes into body prose.

<!-- 2026-07-16 · job 510001 · client: Veda Ray -->
- For legally or medically sensitive niches (sovereign birth, health, finance, legal), capture the client's own compliance-safe self-framing ("this is my lived experience, I'm not a lawyer/CPA/doctor") as an explicit avatar VOICE and IDENTITY marker in the ICP, and flag any adjacent angle the client refuses (here: taxes, the "secret account" sovereign-citizen claim). The ICP is where every downstream asset inherits its guardrails; if the disclaimer framing and the refused angles are not stated in the avatar/voice layer, VSL and ad writers re-import the risky claim each run. This is systemic across the class of regulated niches.
