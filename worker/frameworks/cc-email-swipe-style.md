...
- The sign-off is ALWAYS a three-part block on separate lines:
...
5. CTA link line: the ONE CTA of the email, as its own line with real anchor text. The same destination may repeat 2 to 3 times through the email (early, middle, close), but it is always the SAME action. One CTA, one core idea per email, always.
6. Sign-off: "To your success," on its own line, then the sender name with a topic-matched nickname in quotes: Trent "0%" Kus, Trent "Sequencing" Kus, Trent "Funded" Kus. THE NICKNAME CHANGES EVERY EMAIL and always echoes that email's core idea. (Swipe file precedent: John 'Funding' Duda on the funding email, John 'Broker' Duda on the money-broker email.)
7. P.S. line: mandatory. Either (a) opens the loop for tomorrow's email ("P.S. Tomorrow I'm sending you a case study. Real client. Real results. Don't miss it."), (b) repeats the CTA with scarcity or the live-only bonus, or (c) drops one extra proof point. The P.S. is often hyperlinked to the CTA.
...
- Every email has: subject, pretext, {{ subscriber.first_name }}, one CTA (repeated max 3x, same destination), "To your success," sign-off with a FRESH topic-matched nickname in quotes, and a P.S.
- Formatting pass: bolds on pains/numbers/commands, italics on reframes/quotes, at least one underlined key sentence per email, no bare walls of text.
- One core idea per email. If an email teaches two things, split it.
- Proof anchors repeat across the sequence and match the approved foundation docs exactly, or carry [PROOF:].
- Read the whole sequence in order: P.S. loops must actually connect (if email 3's P.S. promises a case study, email 4 IS that case study).
