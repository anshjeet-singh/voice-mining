...
export const STAGES: Record<string, StageDef> = {
  ...
  more_content_yt: {
    ...
    docs: () => [
      { docType: "content_yt_extra", filename: "01_yt_content.md", title: "YouTube Scripts", description: "An ON-DEMAND batch of long-form YouTube scripts per the youtube-script skill. THE GENERATION REQUEST (how many, plus topic direction or a chosen outlier) is in the REVISION FEEDBACK section. START from the YOUTUBE PACKAGING list in the PROVEN CONTENT ASSETS section of the research (outlier-modeled titles, hooks, and content bullets already validated for this niche) and any Competitor Content Intel document in the approved docs: pick or extend those packages before inventing new ones. Each script: title + thumbnail brief locked first, the 4-beat hook WORD FOR WORD (reinforce packaging, lead with the problem, authority anchor from the CLIENT's rise or price paid, two-part promise), then a story-led body scaffold of 3-5 arcs (each arc: story, tension, teaching payoff, with value + lifestyle + connection layered), the CTA woven in (default: DM keyword lead magnet trigger), and recording notes where the format needs them. Run the skill's anti-pattern audit INTERNALLY and fix everything it catches; do NOT include the audit in the document. Flag at most one line for operator review if something genuinely needs their eye OUTPUT FORMAT: each piece is its OWN unit: start each with a # title line and separate every piece from the next with a line containing exactly <!-- SPLIT --> (the app files each piece as its own card on the content board). No batch-level intro or summary sections: the pieces ARE the deliverable." },
    ],
    extraInstructions: () =>
