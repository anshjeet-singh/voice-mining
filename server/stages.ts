...
export const STAGES: Record<string, StageDef> = {
  ...
  emails: {
    ...
    extraInstructions: (ft) =>
      `This client runs a ${ft === "webinar" ? "WEBINAR funnel: the sequence set is community nurture, pre-webinar show-up, post-webinar replay and close, SMS" : "CALL funnel (VSL into booked call): the sequence set is community nurture, post-booking show-up, no-show and post-call recovery, SMS"}. MODEL THE REFERENCE EMAILS in the pre-call-email-writer and email-campaign-writer skills beat for beat: match their length, voice, and structure before deviating. Quality bar: every email passes the suby-email-machine and emails-and-booking frameworks (subject lines earn the open with curiosity or specificity, never 'Reminder:'), reminder emails ALWAYS deliver standalone value alongside the logistics, and the sequence arc follows the frameworks INDEX email routing (quick wins first, ask later, never abandon non-responders). Use [BOOKING LINK], [VSL LINK], [COMMUNITY LINK], [CALL CONFIRMED VIDEO], and [PROOF: ...] placeholders, never invented URLs or fabricated results. Emails reference the community's actual name and pinned content from the approved Skool docs and carry the core promise from the approved funnel docs word for word. SMS in the client's texting voice: contractions, first name, no corporate tone, no emoji unless the client's voice material uses them.`,
  },
};
