...
export const STAGES: Record<string, StageDef> = {
  ...
  skool: {
    ...
    docs: () => [
      { docType: "skool_free_community", filename: "01_free_community.md", title: "Free Community", description: "The FREE Skool community setup, LEAN. Output ONLY these sections, in this order, each under a '## ' heading, nothing else: (1) COMMUNITY NAMES: exactly 3 name options, each on its own line, niche to the credit/funding journey and ownable. Names only, NO recommendation, NO explanation, NO top-of-funnel/pairing commentary. (2) GROUP TAGLINE: exactly 4-5 lines, each line is one emoji + 2-3 WORDS MAX (never a full sentence), whole tagline under 150 characters (e.g. '\\ud83d\\udcb0 Get funded', '\\u2705 Fix your credit'). (3) ABOUT PAGE: TWO variations labeled 'Variation A' and 'Variation B', EACH under 1000 characters, each following this exact anatomy: an ATTENTION opening line, then the ICP + their main pain point, then ONE line about the owner near the TOP (pull the owner's real story from onboarding, e.g. Trent scaled a 7-figure Airbnb business), then 'What you unlock for free:' as a GREEN-TICK list (every line starts with \\u2705, NO other emojis) that includes the core value items AND exactly THREE named lead magnets, then 'Bonuses:' (green ticks) that ALWAYS include a weekly live Q&A, a personalized 1-on-1 onboarding call, and access to the community, then one line of social proof, then 'Who this is for' / 'Who this is not for' (2-3 ticks/crosses each), then a short urgency CTA ('Only stays free for the next [N] members') with a catchline (like 'Stop getting denied, start getting positioned') and 'Click join below'. Nothing after the CTA. (4) ABOUT-PAGE VSL SCRIPT: a word-for-word ~3-minute (420-480 word) script per the skool-about-vsl framework. (5) CATEGORIES: the standard community categories PLUS only 1-2 EXTRA categories that genuinely add value and are INDEPENDENT of the others (never a category already covered by another, e.g. no 'proof wall' when 'Wins' exists). Names + one short line each. (6) GAMIFICATION LEVELS: just the 9 level names, numbered, cool and gamified. No descriptions, no sentences. (7) PLUGINS / NEXT STEPS: the community links, membership questions, and plugin config live HERE and nowhere else. (8) PINNED POSTS: EXACTLY TWO. Post 1 titled like 'Welcome inside \\u2014 follow the steps below' (welcome to [COMMUNITY NAME], 1-2 lines about the community, then a numbered checklist: 1. Book your 1-on-1 onboarding call [LINK], 2. Sign up for the weekly live Q&A [LINK], 3. Start here [links the Start Here module], 4. Introduce yourself [links Post 2]). Post 2 titled 'Introduce yourself'. NO accountability-buddy post. (9) START HERE MODULE: exactly two lines of copy, no prose \\u2014 'Copy this key into the Skool course import:' then the course key, then 'Here are your 5 free course videos:' then the 5 video titles as a list. (10) SETUP CHECKLIST: a short tick list of setup actions. DO NOT output a community pipeline plan, community foundation section, behavioral-science notes, or content strategy \\u2014 those are banned. Use [COMMUNITY NAME] (capitals) as a placeholder everywhere the name would go; never invent a name in the body copy" },
      { docType: "skool_paid_community", filename: "02_paid_community.md", title: "Paid Community", description: "The PAID Skool community setup, SAME lean section structure and same rules as the free community doc (names, tagline, two <1000-char About pages, VSL script, categories, 9 gamification levels, plugins/next steps, two pinned posts, Start Here, setup checklist), except this community SELLS the exact paid offer (name, price, promise, guarantee) from the approved Offers document, never a variant. The About page's 'What you unlock' is the paid value stack and the CTA points to the paid join/checkout. The 9 GAMIFICATION LEVEL NAMES MUST BE IDENTICAL to the free community's. Same bans (no pipeline plan / foundation / behavioral-science / content-strategy sections). Use [COMMUNITY NAME] as the placeholder" },
    ],
    extraInstructions: () =>
      "LESS IS MORE is the hard rule for this stage: plain, copy-pasteable text a human pastes straight into Skool, ZERO meta-commentary, no 'higher-converting format' notes, no 'next actions', no rationale paragraphs, no audit sections. If a line is not copy that goes live in Skool, delete it. Build BOTH communities. GAMIFICATION LEVEL NAMES ARE IDENTICAL across free and paid: one ladder derived from the client's brand language. Both About-page VSL scripts are word-for-word, ~3 minutes spoken (420-480 words), per the skool-about-vsl framework's beat sequence. Ground the owner line, pains, and proof in the onboarding material and research verbatims. [COMMUNITY NAME] stays a literal capitalized placeholder in all body copy. No em dashes anywhere.",
  },
