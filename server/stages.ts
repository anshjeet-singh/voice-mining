...
export const STAGES: Record<string, StageDef> = {
  ...
  emails: {
    ...
    docs: (ft) => [
      { docType: "email_sequence_14day", filename: "01_email_sequence_14day.md", title: "14-Day Community Sequence", description: `The 14-day email sequence sent to every FREE Skool community joiner. One email per day. Every value email points INTO the free community to consume a SPECIFIC named asset from the approved Skool docs (toolkit, pinned lead magnet, named classroom video): value delivery, belief breaking, pain articulation, urgency, in the swipe-file arc. The CTA warms them toward the offer and the next funnel step: ${ft === "webinar" ? "register for the webinar ([REGISTRATION LINK])" : "book an onboarding call ([VSL LINK])"}. Each email in the framework's block layout: Send / Subject / Preview text each on its own line, then the body` },
