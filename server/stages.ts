...
export const STAGES: Record<string, StageDef> = {
  ...
  ads: {
    ...
    docs: () => [
      { docType: "ad_scripts", filename: "01_angles_and_scripts.md", title: "Ad Angles + Video Scripts", description: "One document, two parts. PART 1, THE ANGLE MATRIX (keep it tight, a page or two): 2-4 sub-avatars from the approved ICP Snapshot, buying reasons worked across the ten categories (functional, emotional, social, financial, time, risk, status, identity, situational, comparative), then the 10 SELECTED batch angles as a ranked table, each tagged with sub-avatar, buying-reason category, hook category, production format, awareness level, and the market verbatim that backs it. No long-list padding: only the thinking that survives into the batch. PART 2, EXACTLY 10 full-length video ad scripts (30-90s spoken, ~60s ideal), numbered, one per selected angle, written word for word: hook, body, CTA. Each script headed by its angle tags and destination. The batch MUST pass the ad-script-system verification checklist: 3+ sub-avatars, 5+ hook categories, 5+ production formats, no hook flows onto another ad's body" },
      { docType: "ad_statics", filename: "02_static_and_broll_ads.md", title: "Static Ads (RENDERED) + B-Roll", description: "10 native static ads, FULLY RENDERED as 1080x1350 PNGs, built and quality-checked per the static-render-rules framework (a HARD contract: read it first and follow every numbered rule). Process: (1) Read the static-ad-builder skill IN FULL including its references/ specs, then VIEW its assets/reference-screenshots and 2-3 approved outputs from past batches in the client's Drive Ads folder with the Read tool BEFORE building anything. (2) Spec each ad: format number and name from the native-formats library, all on-image text verbatim, sub-avatar, angle from the matrix, awareness level, destination. (3) BUILD with the skill's own pipeline (its scripts/, chrome crops, fonts) for every format it covers; ad-factory ('node /Users/anshjeetsingh/ad-factory/render.js <file.html> <outputDir>' for HTML at class 's' 360x450, genimage.js for object/environment imagery only, NEVER people) is the fallback for formats the skill does not cover. (4) Run the VISUAL QA LOOP on every ad: view the rendered PNG, grade against reference screenshots + spec + the native test, fix and re-render until it passes, and log one QA line per ad. (5) Save sources + output/ PNGs + brief.md (house batch format) to '/Users/anshjeetsingh/Library/CloudStorage/GoogleDrive-anshjeets@gmail.com/My Drive/Cashflow Coaches/<Client Name>/Ads/AdsBatch_<YYYY-MM-DD>/'. (6) This deliverable doc contains per-ad specs, the PNG path table, and the QA log. Then 5 b-roll caption ads (on-screen text lines with a footage brief, specs only). Statics reuse the strongest angles from the matrix. On-image CTAs ONLY in the format's designated CTA slot per the spec, never designed CTA text floating on a native format. NEVER AI-generate a human. Real screenshots only for proof formats: where a real client screenshot is required and unavailable, spec it with [PROOF: ...] and DO NOT fabricate the screenshot" },
      { docType: "ad_campaign_plan", filename: "03_campaign_plan.md", title: "Campaign Plan", description: "The Forester content-ad architecture from the haynes-scaling-systems framework sized to THIS client: content inventory audit into Cycle Bin 1 (pure views) and Cycle Bin 2 (strategic, no CTAs), 3-second exclusion setup, retention window from the client's sales cycle, budget split table, naming conventions, plus the DR campaign structure the 10 scripts feed into, and a compliance pre-flight per meta-ad-restrictions-prep for this niche" },
    ],
    extraInstructions: (ft) =>
